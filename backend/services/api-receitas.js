...
export const consultarTodasReceitas = async () => {
  return fs
    .readAsStringAsync(pathReceitasDatabase)
...
export const pesquisarReceitaPorNome = async (nome) => {
  console.log(nome);
  return await consultarTodasReceitas().then((receitas) =>
    receitas.filter((receita) =>
      removerFormatacao(receita.receita).startsWith(removerFormatacao(nome))
    )
  );
};

export const consultarReceitaPorId = async (id) =>
  await consultarTodasReceitas().then((receitas) =>
    receitas.find((receita) => receita.id == id)
  );
